...
public class ServerSession extends AbstractSession {
    ...
    public ServerSession(final ServerFactoryManager server, final IoSession ioSession) throws Exception {
    ...
    public String getNegociated(final int index) {
    ...
    @Override
    public IoWriteFuture writePacket(final Buffer buffer) throws IOException {
    ...
    private void running(final SshConstants.Message cmd, final Buffer buffer) throws Exception {
        switch (cmd) {
    ...
    private void receiveKexInit(final Buffer buffer) throws IOException {
        clientProposal = new String[SshConstants.PROPOSAL_MAX];
        I_C = receiveKexInit(buffer, clientProposal);
    }

    private void serviceRequest(final Buffer buffer) throws Exception {
        String request = buffer.getString();
        log.debug("Received SSH_MSG_SERVICE_REQUEST '{}'", request);
    ...
    private void userAuth(Buffer buffer, final SshConstants.Message cmd) throws Exception {
     ...
     */
    protected int getActiveSessionCountForUser(final String userName) {
        int totalCount = 0;
    ...
    private void channelOpen(final Buffer buffer) throws Exception {
    ...
    public String createX11Display(final boolean singleConnection, final String authenticationProtocol, final String authenticationCookie, final int screen) throws IOException {
