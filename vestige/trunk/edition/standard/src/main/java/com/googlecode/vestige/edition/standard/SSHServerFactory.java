...
public class SSHServerFactory implements Callable<SshServer> {
    ...
    public SSHServerFactory(final File sshBase, final File sshHome, final SSH ssh, final File appHomeFile, final ApplicationManager applicationManager, final VestigePlatform vestigePlatform) {
    ...
    public SshServer call() throws Exception {
        ...
        if (!privateKey.exists()) {
            privateKey = new File(sshHome, "vestige_rsa");
            if (!privateKey.exists()) {
                sshBase.mkdirs();
                privateKey = new File(sshBase, "vestige_rsa");
                IOUtils.copy(StandardEditionVestige.class.getResourceAsStream("vestige_rsa"),
        ...
        if (!authorizedKeysFile.exists()) {
            authorizedKeysFile = new File(sshHome, "authorized_keys");
            if (!authorizedKeysFile.exists()) {
                sshBase.mkdirs();
